...
const Add = ({token}) => {
  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex flex-col w-full items-start gap-3'>
      <div>
        <p className='mb-2'>Képek feltöltése</p>
      ...
      <div className='w-full'>
        <p className='mb-2'>Termék leírása</p>
        <textarea onChange={(e)=>setDescription(e.target.value)} value={description} className='w-full max-w-[500px] px-3 py-2' type="text" required />
      </div>

      <div className='flex flex-col sm:flex-row gap-2 w-full sm:gap-8'>
        <div>
          <p className='mb-2'>Termékkategória</p>
        ...
        <div>
          <p className='mb-2'>Alkategória</p>
          <select onChange={(e)=>setSubCategory(e.target.value)} className='w-full px-3 py-2 '>
            <option value="Topwear">Típus-1</option>
            <option value="Bottomwear">Típus-2</option>
            <option value="Winterwear">Típus-3</option>
          </select>
        </div>

        <div>
          <p className='mb-2'>Termék ára</p>
          <input onChange={(e)=>setPrice(e.target.value)} value={price} className='w-full px-3 py-2 sm:w-[120px]' type="number" />
        </div>
      </div>

      <div>
        <p className='mb-2'>Termék méretei</p>
        <div className='flex gap-3'>
          <div onClick={()=>setSizes(prev => prev.includes("S") ? prev.filter(item => item !== "S") : [...prev,"S"])} value={name}>
      ...
      <div className='flex gap-2 mt-2'>
        ...
        <label className='cursor-pointer' htmlFor="bestseller">Legtöbbet eladottnak jelölés</label>
      </div>

      <button className='w-28 py-3 mt-4 bg-black text-white' type="submit">HOZZÁADÁS</button>
